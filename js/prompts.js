...
- 里程碑是可检验的结果并带时间点（结合【当前时间】推算，粒度用「X月中 / X月底」，不要用今天、明天、本周几这种紧日期），第一个在 2-4 周内可达成，其余按 horizon 匀开。
- 节奏必须现实：用户的时间同时分给多个目标和本周要事（看上下文里的预算与实际投入），不要把这个目标当成用户唯一的事，宁可排松不要排挤。
- firstActions 是 2-3 个「下次有空的 30-90 分钟就能做」的具体动作，不指定哪天做——把事情排到具体某天是周计划和日计划的职责，不是这里的。
- weeklyHours 结合用户现有目标与作息给现实数字。
